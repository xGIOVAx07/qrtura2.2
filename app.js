// Simulamos que tomamos la ubicación actual del usuario
function simulateUserLocation() {
    // Datos de ubicación simulada (por ejemplo, una latitud y longitud de Tepic)
    const simulatedLocation = {
        lat: 21.5, // latitud de Tepic
        lon: -104.9 // longitud de Tepic
    };

    // Mostramos la ubicación simulada en la página
    const locationDisplay = document.getElementById("userLocation");
    locationDisplay.textContent = `Ubicación actual: Lat: ${simulatedLocation.lat}, Lon: ${simulatedLocation.lon}`;

    // Devolvemos la ubicación simulada
    return simulatedLocation;
}

// Llamamos a la función para simular la ubicación al cargar la página
...
function showCredentialOption() {
    ...
    document.getElementById("credentialOption").style.display = (passengerType === "student" || passengerType === "child" || passengerType === "senior" || passengerType === "disabled") ? "block" : "none";
}
...
function showArrivalTimeAndFare() {
    ...
    result.textContent = `El camión ${bus} llegará a las ${arrivalTime}, tomando aproximadamente ${travelTime} minutos desde tu ubicación. El costo del pasaje es de $${fare} pesos.`;
}
